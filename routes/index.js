...
/* 获取所有分类 */
router.get('/api/allcategorys', (req, res) => {
  let sqlStr = 'SELECT * FROM category'

  conn.query(sqlStr, (error, results, fields) => {
    if (error) {
      console.log(error)
      res.json({ err_code: 0, message: '请求分类数据失败' })
    } else {
      results = JSON.parse(JSON.stringify(results))
      res.json({ success_code: 200, message: results })
    }
  ...
  conn.query(cateSqlStr, (error, results, fields) => {
    if (!error) {
      ...
      for (let i = 0; i < results[0]['COUNT(*)']; i++) {
        sqlStr +=
          'SELECT * FROM recommend WHERE category = ' + (i + 1) + ' LIMIT 4;'
      }
      conn.query(sqlStr, (error, results, fields) => {
        if (!error) {
          results = JSON.parse(JSON.stringify(results))
 ...
 * 删除分类
 */
router.post('/api/delete_category', (req, res) => {
  // 获取数据
  const cate_id = req.body.cate_id

  let sqlStr = 'DELETE FROM category WHERE cate_id =' + cate_id
  conn.query(sqlStr, (error, results, fields) => {
    if (error) {
      console.log(error)
      res.json({ err_code: 0, message: '删除失败!' })
 ...
 * 修改分类信息
 */
router.post('/api/update_category', (req, res) => {
  // 获取数据
  const cate_id = req.body.cate_id
  const cate_name = req.body.cate_name

  let sqlStr = 'UPDATE category SET cate_name = ? WHERE cate_id = ' + cate_id
  let strParams = [cate_name]
  conn.query(sqlStr, strParams, (error, results, fields) => {
    if (error) {
      console.log(error)
      res.json({ err_code: 0, message: '修改失败!' })
...
router.post('/api/add_category', (req, res) => {
  ...
  form.parse(req, (err, fields, files) => {
    ...
    let cate_id = fields.cate_id
    let cate_name = fields.cate_name
    let add_sql = 'INSERT INTO category(cate_id, cate_name) VALUES (?, ?)'
    let sql_params = [cate_id, cate_name]
    conn.query(add_sql, sql_params, (error, results, fields) => {
      if (error) {
        console.log(error)
        res.json({ err_code: 0, message: '加入失败!' })
...
// 添加订单
router.post('/api/add_order', (req, res, next) => {
    // 获取客户端传过来的商品信息
    let num = parseInt(req.body.num)
    let price = parseInt(req.body.price)
    let total = parseInt(req.body.total)
    let g_id = parseInt(req.body.g_id)
    let u_id = parseInt(req.body.u_id)
    let add_sql =
      'INSERT INTO orders (num, price, total, g_id, u_id) VALUES (?, ?, ?, ?, ?)'
    let sql_params = [num, price, total, g_id, u_id]
    conn.query(add_sql, sql_params, (error, results, fields) => {
      if (error) {
        console.log(error)
        res.json({ err_code: 0, message: '添加订单失败!' })
      } else {
        res.json({ success_code: 200, message: '添加订单成功!' })
      }
    })
  })
  // 添加订单
router.get('/api/get_order', (req, res, next) => {
  if (req.query.params == '0' || req.query.params == '1') {
    let add_sql =
      'SELECT *, orders.id as o_id FROM orders, user_info, recommend where orders.u_id = user_info.id and orders.g_id = recommend.goods_id and orders.state = ' +
      parseInt(req.query.params)
    conn.query(add_sql, (error, results, fields) => {
      if (error) {
        console.log(error)
        res.json({ err_code: 0, message: '查询订单失败!' })
      } else {
        res.json({ success_code: 200, message: '查询订单成功!', data: results })
      }
    })
  } else {
    let add_sql =
      'SELECT *, orders.id as o_id FROM orders, user_info, recommend where orders.u_id = user_info.id and orders.g_id = recommend.goods_id'
    conn.query(add_sql, (error, results, fields) => {
      if (error) {
        console.log(error)
        res.json({ err_code: 0, message: '查询订单失败!' })
...
router.get('/api/delete_order', (req, res, next) => {
  const id = req.query.id
  console.log('id', id)
  let add_sql = 'DELETE FROM orders where id = ' + id
  conn.query(add_sql, (error, results, fields) => {
    if (error) {
      console.log(error)
      res.json({ err_code: 0, message: '删除订单失败!' })
    } else {
      res.json({ success_code: 200, message: '删除订单成功!' })
    }
  })
})

router.get('/api/update_order', (req, res, next) => {
  const id = req.query.id
  const state = req.query.state
    // console.log("id", id);
  let add_sql = 'UPDATE orders set state = ' + state + ' where id = ' + id
  conn.query(add_sql, (error, results, fields) => {
    if (error) {
      console.log(error)
      res.json({ err_code: 0, message: '更新订单失败!' })
    } else {
      res.json({ success_code: 200, message: '更新订单成功!' })
    }
  })
})
